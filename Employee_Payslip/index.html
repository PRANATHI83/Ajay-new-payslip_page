...
<body>
     <header>
        ...
        <h1>Employee Payslip Tracker</h1>
    </header>
    <div class="container">
        ...
        <div class="form-container" id="form-container">
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" id="employee-id" placeholder="e.g., ATS0xxx" maxlength="7">
            ...
            <div class="payslip">
                <div class="payslip-header">
                    <div class="header-top">
                        ...
                        <img src="ATS Wallpaper.jpg" alt="ATS Logo" class="company-logo" />
                    ...
                    <h3>Payslip for the month of <span id="preview-month-year"></span></h3>
                </div>
                ...
                <div class="payslip-details">
                    <table class="details-table">
                        <tr>
                            ...
                            <td><span class="detail-label">Employee Type:</span> <span id="preview-type"></span><span class="detail-label" style="border-left:2px solid #9e9e9e;padding-left: 5px; margin-left: 5px;">Employee ID:</span> <span id="preview-id"></span></td>
                        </tr>
                        <tr>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (window.location.protocol === 'file:') {
                showNotification('error', 'This application must be run via the server. Please open http://13.201.99.105:3993/Employee_Payslip instead of the local file.');
            ...
            logoImg.addEventListener('error', function(event) {
                ...
                showNotification('error', 'Failed to load logo. Using placeholder image. Please ensure the server is running and access via http://13.201.99.105:3993/Employee_Payslip.');
                logoImg.src = 'https://placehold.co/180x100?text=Logo';
            ...
            async function viewPayslip() {
                ...
                if (validateForm()) {
                    ...
                    try {
                        const response = await fetch('http://13.201.99.105:3993/api/payslips?employeeId=' + employeeId + '&month=' + month + '&year=' + year);
                        if (!response.ok) {
    ...
    try {
        ...
        function convertWholeNumber(num) {
            ...
            if (crore > 0) {
                if (crore < 20) {
                    words = first[crore] + 'Crore ';
                } else {
                    const croreTens = Math.floor(crore / 10);
                    const croreOnes = crore % 10;
                    words = tens[croreTens] + (croreOnes > 0 ? ' ' + first[croreOnes] : '') + ' Crore ';
                }
                num = num % 10000000;
            }
            ...
            if (lakh > 0) {
                if (lakh < 20) {
                    words += first[lakh] + 'Lakh ';
                } else {
                    const lakhTens = Math.floor(lakh / 10);
                    const lakhOnes = lakh % 10;
                    words += tens[lakhTens] + (lakhOnes > 0 ? ' ' + first[lakhOnes] : '') + ' Lakh ';
                }
            }
            ...
            if (thousand > 0) {
                if (thousand < 20) {
                    words += first[thousand] + 'Thousand ';
                } else {
                    const thousandHundreds = Math.floor(thousand / 100);
                    const thousandTens = Math.floor((thousand % 100) / 10);
                    const thousandOnes = thousand % 10;
                    words += (thousandHundreds > 0 ? first[thousandHundreds] + 'Hundred ' : '') +
                             (thousandTens > 1 ? tens[thousandTens] + (thousandOnes > 0 ? ' ' + first[thousandOnes] : '') : 
                             (thousandTens === 1 ? first[thousand % 100] : first[thousandOnes])) + ' Thousand ';
                }
            }
            ...
            if (num > 0) {
                if (num < 20) {
                    ...
                    const tensValue = Math.floor((num % 100) / 10);
                    const onesValue = num % 10;
                    if (hundreds > 0) {
                        words += first[hundreds] + 'Hundred ';
                    }
                    if (tensValue > 0) {
                        words += tens[tensValue] + (onesValue > 0 ? ' ' + first[onesValue] : '');
                    } else if (onesValue > 0) {
                        words += first[onesValue];
                    }
                }
            }
